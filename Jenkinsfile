pipeline {
  ...
  environment {
      ...
      DOCKER_SIGNER_IMG = 'eu.gcr.io/metaco/devops/docker:signer'
      DOCKER_CONTENT_TRUST_SERVER = 'https://registry.metaco.network:4443'
      DOCKER_IMG = 'eu.gcr.io/metaco/silo/nbxplorer'
  ...
  parameters {
    ...
    booleanParam(name: 'DOCKER_BETA', defaultValue: false, description: 'Deploy images to BETA channels?')
    booleanParam(name: 'DOCKER_ALPHA', defaultValue: false, description: 'Shall build the docker images?')
  }
    ...
    stage('Deploy Docker image') {
      when {
        anyOf {
          ...
          expression { params.DOCKER_ALPHA == true }
          tag "*"
        }
      }
      steps {
          script {
            ...
            docker.withRegistry("https://eu.gcr.io","gcr:metaco"){
              sh '''
    ...
    stage('Build Docker for beta channels') {
      ...
      steps {
        ...
        sh '''
          ...
          docker build -t ${DOCKER_IMG_BETA}:${DOCKER_TAG} -f Dockerfile.linuxamd64 .
          docker push ${DOCKER_IMG_BETA}:${DOCKER_TAG}
        '''
      }
    }
        ...
        script {
          ...
          docker.withRegistry("https://eu.gcr.io","gcr:metaco"){
            sh '''
                ...
                docker build -t ${DOCKER_CONTENT_TRUST_REPOSITORY}:${DOCKER_TAG} -f Dockerfile.linuxamd64 .
                docker run --rm \
                ...
                ${DOCKER_SIGNER_IMG} docker push ${DOCKER_CONTENT_TRUST_REPOSITORY}:${DOCKER_TAG}
            '''
          }
        }
      }
